...
function DoctorSchedule(props) {
  ...
  let updateSchedule = async () => {
    setLoading(true);
    let data = await handleGetScheduleByDateApi(
  ...
  let getSelectDate = () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      object.label = capitalizeFirstLetter(
        moment(newDate).add(i, "days").locale("vi").format("dddd - DD/MM")
      );
  ...
  return (
    <>
      <div className="doctor-schedule">
        ...
        <div>
          <i className="fa-duotone fa-calendar-days"></i> 診療スケジュール
        </div>
        <div
          className="doctor-schedule-t"
        ...
        >
          <div className="doctor-schedule-button">
            ...
            <div>
              洗濯 <i className="fa-regular fa-hand-back-point-up"></i> で予約
              　　(スケジュールコストは、無料です)
            </div>
          </div>
          <div className="doctor-schedule-info">
            ...
            {!toggleDetailPrice && <b>{formatCurrency(price)} đ</b>}
            {!toggleDetailPrice && (
              <span className="show-detail" onClick={handleToogleDetailPrice}>
                Xem chi tiết
              </span>
            )}
              ...
              <div>
                <div className="show-detail-div">
                  ...
                  <div className="payment">
                    Phòng khám có hình thức thanh toán bằng tiền mặt và quẹt thẻ
                  </div>
                </div>
                <div className="show-detail" onClick={handleToogleDetailPrice}>
                  Ẩn bảng giá
                </div>
              </div>
            )}
      ...
      {toggleBooking && (
        <BookingModel
          ...
          handleToogleBookingFarent={handleToogleBooking}
          info={itemBooking}
          price={price}
          doctorName={props.doctorName}
          doctorPosition={props.doctorPosition}
