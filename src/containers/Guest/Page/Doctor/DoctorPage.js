...
function DoctorPage() {
  ...
  let getData = async () => {
    ...
    setDoctorName(
      data.data.userData.lastName + " " + data.data.userData.firstName
    );
    ...
    let reviewData = await handleGetReviewByDoctorIdApi(
  ...
  useEffect(() => {
    async function countView() {
      let data = await countViewDoctorApi(
        window.location.pathname.split("/")[2]
      );
      console.log("countViewDoctorApi: ", data);
    }
    countView();
  }, [loading]);
      ...
      <div className="doctor-detail-container">
        <div className="doctor-info">
          ...
          <div className="doctor-info-content">
            ...
            <h6 className="doctor-description">{doctorDescription}</h6>
            <div className="doctor-rate">
              <div className="doctor-rate-star">
                {rateDoctor === "Chưa có đánh giá" ? (
                  rateDoctor
                ) : (
                  <Fragment>
                    {
            ...
            {review.map((item, index) => {
              return (
                <div className="doctor-review-item">
                  <div className="doctor-review-item-header">
                    <div
                      className="doctor-review-item-avatar"
                      style={{
                        backgroundImage: `url(${item.bookingData.patientData.userData.image})`,
                      }}
                    ></div>
                    <div className="doctor-review-item-name">
                      {item.bookingData.patientData.userData.lastName}{" "}
                      {item.bookingData.patientData.userData.firstName}
                    </div>
                    <div className="doctor-review-item-date">
                      <i className="far fa-clock"></i>
                      Đã khám ngày{" "}
                      {moment(item.bookingData.date).format("DD/MM/YYYY")}
                    </div>
                  </div>
                  ...
                  <div className="doctor-review-item-rating">
                    <i
                      class={
                        item.rate >= 1
                          ? "fa-solid fa-star icon-rating"
                          : "fa-regular fa-star icon-rating"
                      }
                    ></i>
                    <i
                      class={
