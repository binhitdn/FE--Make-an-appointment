...
function DetailSpecialty() {
  ...
  let getData = async () => {
    ...
    let data = await handleGetSpecialityByIdApi(
    ...
    let doctors = await handleGetDoctorBySpecialtyApi(
      window.location.pathname.split("/")[2]
    );
  ...
  let getSelectDate = () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      object.label = capitalizeFirstLetter(
        moment(new Date()).add(i, "days").locale("vi").format("dddd - DD/MM")
      );
    ...
    <>
      <div className="detail-speciality-container">
        <div className="detail-speciality-content">
          <div
            className="speciality-background"
            style={{ backgroundImage: `url(${image})` }}
          >
            <div className="speciality-background-opacity">
              <div className="speciality-background-content">
                <div className="speciality-name">{name}</div>
                <div
                  className="speciality-description"
                  style={styleDescription()}
                >
                  <div
                    dangerouslySetInnerHTML={{ __html: contentHTML }}
                    className="speciality-description-child"
                    // style={
                    //     styleDescription
                    // }
                  ></div>
                ...
                <div>
                  {isHiddenDescription ? (
                    <div
                      className="speciality-description-button"
                      onClick={() => setIsHiddenDescription(false)}
                    >
                      Xem thêm
                    </div>
                  ) : (
                    <div
                      className="speciality-description-button"
                      onClick={() => setIsHiddenDescription(true)}
                    >
                      Thu gọn
                    </div>
                  )}
          ...
          <div className="list-doctor-by-speciality">
            <div className="list-doctor-by-speciality-child">
              <select
                className="filter-doctor-location"
                onChange={(e) => {
              ...
              <div className="doctors-by-speciality">
                {handleFilterDoctor(doctors).length > 0 ? (
                  handleFilterDoctor(doctors).map((doctor, index) => {
                    return (
                      <Link
                        className="doctor-by-speciality"
                        key={index}
                        to={"/doctor/" + doctor.id}
                      >
                        <div className="doctor-by-speciality-left">
                          <div className="doctor-by-speciality-avatar">
                            <div
                              className="avatar"
                              style={{
                                backgroundImage: `url(${doctor.userData.image})`,
                              }}
                            ></div>
                          </div>
                          <div className="doctor-by-speciality-content">
                            <div className="doctor-by-speciality-name">
                              ...
                              {doctor.userData.lastName}{" "}
                              {doctor.userData.firstName}
                            </div>
                ...
                <div>
                  ...
                  <div className="no-doctor">
                    <div
                      className="no-doctor-image"
                      style={{
                        backgroundImage: `url("https://wallpaperaccess.com/full/4620961.png")`,
